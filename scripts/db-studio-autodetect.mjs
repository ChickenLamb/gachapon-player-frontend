 ...
 */
async function main() {
	console.log('🔍 Looking for local D1 database...');

	// Step 1: Find the database file (or use manual override)
	let dbPath = MANUAL_DB_PATH;

	if (dbPath) {
		console.log(`📌 Using manual database path: ${dbPath}`);

		// Validate manual path exists
		if (!existsSync(dbPath)) {
			console.error(`❌ Error: Manual database path does not exist: ${dbPath}`);
			console.error('');
	...
	if (!dbPath) {
		console.error('❌ Error: Local D1 database not found!');
		console.error('');
		console.error('Please ensure you have run one of the following:');
		console.error('  • pnpm run dev        (starts dev server with miniflare)');
		console.error('  • pnpm run preview    (starts preview with wrangler)');
		console.error('');
	...
	if (!isValid) {
		console.error('❌ Error: Database exists but is not accessible via wrangler!');
		console.error('');
		console.error('Please ensure:');
		console.error(`  • wrangler.jsonc has correct d1_databases configuration`);
		console.error(`  • database_name matches "${DATABASE_NAME}"`);
		console.error('');
		process.exit(1);
	}
	...
	console.log(`📝 Updating ${CONFIG_FILE}...`);
	updateConfig(dbPath);

...
const participation = await fetch(`/api/v1/merchant-events/${eventId}/participation/${userId}`, {
	headers: { Authorization: `Bearer ${token}` }
});
...
```typescript
// My Events page
const participations = await fetch(`/api/v1/merchant-events/user/${userId}/participations`, {
	headers: { Authorization: `Bearer ${token}` }
});
```
...
const redemption = await fetch(`/api/v1/merchant-events/${eventId}/redeem/${userId}`, {
	method: 'POST',
	headers: { Authorization: `Bearer ${token}` }
});
...
const MOCK_TOKEN =
	'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.';
...
export class QRCodeService {
	...
	async generateQRCode(ttlMinutes: number = 2): Promise<QRCode> {
		const response = await this.client.post<QRCode>('/user-activity/qr-codes/generate', {
			ttlMinutes
		});
		return response.data;
	}
	...
	async validateQRCode(code: string): Promise<QRCodeValidation> {
		const response = await this.client.post<QRCodeValidation>('/user-activity/qr-codes/validate', {
			code
		});
		return response.data;
	}
...
export class PaymentService {
	...
	async previewPayment(request: {
		...
		const response = await this.client.post<PaymentPreview>('/payments/preview', request);
		return response.data;
	}
	...
	async cancelPayment(paymentId: string): Promise<Payment> {
		const response = await this.client.post<Payment>(`/payments/${paymentId}/cancel`);
		return response.data;
	}
...
export class ProductService {
	...
	async getProductsByCategory(categoryId: string): Promise<Product[]> {
		const response = await this.client.get<Product[]>(`/products/category/${categoryId}`);
		return response.data;
	}
	...
	async getActiveCategories(): Promise<Category[]> {
		const response = await this.client.get<Category[]>('/categories/root/active');
		return response.data;
	}
}
```
...
export class EventService {
	...
	async getEvent(eventId: string): Promise<Event> {
		const response = await this.client.get<Event>(`/merchant-events/${eventId}`);
		return response.data;
	}
	...
	async joinEvent(eventId: string, userId: string): Promise<EventParticipation> {
		const response = await this.client.post<EventParticipation>('/merchant-events/join', {
			eventId,
			userId
		});
		return response.data;
	 ...
	 */
	async getParticipation(eventId: string, userId: string): Promise<EventParticipation> {
	 ...
	 * Redeem event reward
	 */
	async redeemReward(eventId: string, userId: string): Promise<EventParticipation> {
...
export function initializeAPI(config: { baseUrl: string; onUnauthorized?: () => void }) {
...
export class MockDataGenerator {
	...
	static events(count: number = 3): Event[] {
		return Array.from({ length: count }, (_, i) => ({
			...
			eventName: ['Christmas Special - Free Play', 'New Year Bonus', 'Lucky Draw Weekend'][i],
...
export class MockAPIClient {
	...
	async getMachines(params?: any): Promise<ApiResponse<PaginatedResponse<Machine>>> {
		...
		const filtered = params?.status ? machines.filter((m) => m.status === params.status) : machines;

		return this.createResponse(this.createPaginatedResponse(filtered, params?.page, params?.size));
	}
	...
	async getProducts(params?: any): Promise<ApiResponse<PaginatedResponse<Product>>> {
		...
		return this.createResponse(this.createPaginatedResponse(products, params?.page, params?.size));
	}
	...
	async getEvents(params?: any): Promise<ApiResponse<PaginatedResponse<Event>>> {
		...
		return this.createResponse(this.createPaginatedResponse(events, params?.page, params?.size));
	}
